...
START_TEST(pow_test_1) {
  long double a;
  long double b;
  for (double k = -9.3; k <= 9; k += 1) {
    for (double g = -5.4; g < 5; g += 4.5) {
      a = s21_pow(k, g);
      b = pow(k, g);
...
START_TEST(pow_test_3) {
  ck_assert_double_eq_tol(pow(s21_M_PI, 500), s21_pow(s21_M_PI, 500), 1e248);
  ck_assert_double_eq_tol(pow(1, 0), s21_pow(1, 0), 1e-7);
  ck_assert_double_eq(pow(-1, 3), s21_pow(-1, 3));
  ck_assert_double_eq(pow(-1, 4), s21_pow(-1, 4));
  ck_assert_double_eq(pow(-1, 400), s21_pow(-1, 400));
...
START_TEST(sin_test_2) {
  for (double i = -s21_M_PI * 2; i <= s21_M_PI * 2; i += s21_M_PI / 18) {
